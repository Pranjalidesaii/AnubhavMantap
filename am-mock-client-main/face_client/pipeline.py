...
def _open_picamera2_source(device=0,size=(640, 480)):
    try:
    ...
    picam2 = Picamera2(camera_num=device)
...
class FaceRecognitionClient:
    ...
    def run_camera(self, pearl: bool = False) -> None:
        ...
        if backend == "picamera2":
            read_frame, release_source = _open_picamera2_source(device)
            ...
            while True:
                ...
                if frame_no % frame_skip == 0:
                    ...
                    if detection is not None:
                        ...
                        # Only a recognised (known) face triggers the microscope
                        if name:
                            known_face = name
                    else:
                        last_bbox = None
                ...
                if known_face:
                    logger.info("Recognised '%s' — stopping camera for microscope", known_face)
                    break
        finally:
            release_source()
